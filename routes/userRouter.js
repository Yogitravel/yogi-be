...
router.get("/myvideos", auth, async function (req, res) {
	// my programs = req.user.programs
	const foo = req.user.programs.map((proID) => Programdetail.find({ program: proID }));
	const allVideos = await Promise.all(foo); // all Pro detail docs
